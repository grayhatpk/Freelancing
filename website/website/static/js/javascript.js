    ...
    if ( searchValue === ''){
     ...
     // select all the paragraphs from the pictures div 
      ...
      if (isTextFound) {                                                        // Then disappear all the sections and divs to display the serached images. 
         ...
         buy.addEventListener('click', function () { window.location.href = '/profile'; })
      } 
      else {
          alert('The searched image is not present');
}}}

  const buyvar = document.querySelectorAll('#buy');
  
  buyvar.forEach(button => {
    button.addEventListener('click', function(){
      window.location.href = '/profile';
    });});
  
   const checkbox = document.getElementById('ball-checkbox');
    checkbox.addEventListener('change', function () {
        if (this.checked) {
          document.body.style.backgroundColor = '#86A789';
    ...
    document.querySelector('#contactSubmit').onclick = function(){
      alert('Recorded! Our Team will contact you shortly!');
